...
| `options.assetsPath` | string |  | The address of the static resource file that the plugin depends on (optional).<br>- Publish the node_modules/trtc-sdk-v5/assets directory to CDN or static resource server.<br>- Set the assetsPath parameter to the CDN address, for example: `TRTC.create({ assetsPath: '' })`, the SDK will load the relevant resource files on demand. |
