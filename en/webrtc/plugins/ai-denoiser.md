...
This plugin relies on static resource files. To ensure the browser can load and run these files properly, complete the following steps:
  ...
  2. Pass your CDN address to the `assetsPath` parameter when creating the TRTC instance, for example: `TRTC.create({ assetsPath: '' })`. The SDK will load the relevant resource files as needed.
...
```javascript
const trtc = TRTC.create({ assetsPath: '' });
