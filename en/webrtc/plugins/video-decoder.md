...
The plugin relies on some static resource files. To ensure that the browser can load and run these files properly, you need to complete the following steps:
  ...
  2. Pass your CDN address to the `assetsPath` parameter when creating the `trtc` instance, for example: `TRTC.create({ assetsPath: '' })`. The SDK will load the required resource files as needed.
...
const trtc = TRTC.create({ plugins: [TRTCVideoDecoder], assetsPath: '' });
```
