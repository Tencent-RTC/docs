...
2.  Pass your CDN address to the `assetsPath` parameter when creating the TRTC instance, e.g., `TRTC.create({ assetsPath: '' })`. The SDK will load the necessary resource files on demand.
...
const trtc = TRTC.create({ 
  ...
  assetsPath: ''  // Replace with your static resource address
});
```
