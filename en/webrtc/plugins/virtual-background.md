...
2. Pass your CDN URL to the `assetsPath` parameter when creating the `trtc` instance, for example: `TRTC.create({ assetsPath: '' })`. The SDK will load the required resource files as needed.
...
const trtc = TRTC.create({ plugins: [VirtualBackground], assetsPath: '' });
```
...
| src        | `string`  | Required if type is `image`  | Image URL, such as `` |
